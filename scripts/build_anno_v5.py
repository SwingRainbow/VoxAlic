"""Generate 400-frame annotation HTML for CNN v5 training."""
...
unlabeled_tf = [p for p in tf.glob('*.png') if p.name not in labeled]
unlabeled_lf = [p for p in lf.glob('*.png') if p.name not in labeled]
print(f'Unlabeled: {len(unlabeled_tf)} high + {len(unlabeled_lf)} low')

random.seed(42)
random.shuffle(unlabeled_tf)
random.shuffle(unlabeled_lf)
selected = unlabeled_tf[:325] + unlabeled_lf[:75]
...
<body>
<div id="topbar">
  <span><b>CNN v5 标注</b> — 4位MMSS</span>
...
<div id="main">
  ...
  <div id="export-area" style="display:none;">
    <p style="font-size:13px;color:#aaa;">CSV (存为 annotations_v5.csv):</p>
...
function exportCSV() {{
    ...
    a.href = url; a.download = 'annotations_v5.csv'; a.click();
...
out = Path(__file__).parent / 'annotate_cnn_v5.html'
out.write_text(html, encoding='utf-8')
print(f'Written: {out} ({out.stat().st_size//1024} KB)')
print('Open in browser to annotate.')
