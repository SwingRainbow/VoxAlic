"""Train single-digit CNN with COLON-ANCHORED patch extraction.

Instead of fixed-position or NCC-digit-detection, this finds the colon ":"
in the timer display and uses it as a spatial anchor.

Colon detection: scan center region for two vertically-aligned bright dots.
Patches are cropped at fixed offsets from the colon:
  - 分十: colon X - 40px
  - 分个: colon X - 20px
  - 秒十: colon X + 15px
  - 秒个: colon X + 35px

This adapts to HUD jitter (colon moves → all positions move) without
depending on NCC digit detection quality.
...
BEST_PTH = SCRIPT_DIR / "best_cnn_v4.pth"
...
def find_colon(gray, est_cx, est_cy):
    """Find the colon ':' near the estimated center position.

    Colon = two bright dots (~3×3 px each) vertically aligned with
    a dark gap (~6-8 px) between them.
    Returns (cx, cy) of colon center, or falls back to estimated position.
    """
    h, w = gray.shape
    # Search window: ±20px around estimated position
    x0 = max(5, est_cx - 20)
    x1 = min(w - 5, est_cx + 20)
    y0 = max(5, est_cy - 20)
    y1 = min(h - 5, est_cy + 20)

    best_score = -1.0
    best_cx, best_cy = est_cx, est_cy

    for cy in range(y0, y1, 2):
        for cx in range(x0, x1, 2):
            # Check for two bright dots with gap
            # Upper dot: 3x3 region, mean brightness
            ...
            # Colon pattern: bright upper, dark middle, bright lower
...
def crop_cnn_patch(gray, cx, cy, size=40):
    """Crop size×size centered at (cx, cy), resize to 24×24."""
    h, w = gray.shape
    half = size // 2
    patch = np.zeros((size, size), dtype=np.float32)
    ox0 = max(0, cx - half); oy0 = max(0, cy - half)
    ox1 = min(w, cx + half); oy1 = min(h, cy + half)
    px0 = ox0 - (cx - half); py0 = oy0 - (cy - half)
...
# ── Dataset generation (colon-anchored) ─────────────────────
def generate_dataset(annotated, augment=True):
    """Extract digit patches using colon as spatial anchor."""
    X_list, y_list = [], []
    ...
    for fname, (label_str, path) in annotated.items():
        ...
        # Estimate colon position (center of ROI, between minute and second)
        mid_y = h // 2
        est_colon_x = w // 2
        colon_cx, colon_cy = find_colon(gray, est_colon_x, mid_y)

        # Check if colon detection found something plausible
        # (colon should be near center, within ±30px of estimate)
        if abs(colon_cx - est_colon_x) > 30 or abs(colon_cy - mid_y) > 30:
            colon_missed += 1
            # Fallback to estimated positions
            colon_cx = est_colon_x
            colon_cy = mid_y
        else:
            colon_found += 1
...
def main():
    ...
    print("Generating training patches (colon-anchored)...")
