"""Generate v7 annotation HTML for CNN training — 400 frames."""
...
sel = ut[:142] + ul[:258]; random.shuffle(sel)
print(f'Selected {len(sel)} ({min(142,len(ut))} high + {min(258,len(ul))} low)')

images = []
for i, p in enumerate(sel):
    images.append({'name': p.name, 'b64': base64.b64encode(p.read_bytes()).decode()})
    if (i + 1) % 100 == 0: print(f'  {i + 1}/{len(sel)}')
...
<body>
<div id="topbar">
  <span><b>CNN v7 标注</b> — 4位MMSS ({len(labeled)}已标)</span>
...
<div id="main">
  ...
  <div id="export-area" style="display:none;">
    <p style="font-size:13px;color:#aaa;">CSV (存为 annotations_v7.csv):</p>
...
function exportCSV() {{
    ...
    a.href = url; a.download = 'annotations_v7.csv'; a.click();
...
out = Path(__file__).parent / 'annotate_cnn_v7.html'
