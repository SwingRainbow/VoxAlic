"""Generate annotation HTML for 300 unlabeled high-score frames."""
import os, sys, base64, json, random
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from train_cnn_v2 import load_annotations
...
all_tf = [p for p in TF.glob('*.png') if p.name not in labeled]
random.seed(42)
random.shuffle(all_tf)
selected = all_tf[:300]
print(f'Selected {len(selected)} from {len(all_tf)} unlabeled')
...
<body>
<div id="topbar">
  <span><b>CNN v3 标注</b> — 4位MMSS</span>
...
<div id="main">
  ...
  <div id="hint">输入 <b>MMSS</b> 如03:52→0352。回车确认，Tab跳过，Ctrl+Z撤销，Ctrl+←→导航</div>
  <div id="input-row">
...
function undo() {{
    let e = IMAGES[idx]; delete state.labels[e.name]; delete state.skipped[e.name];
    saveState(); document.getElementById('status').textContent = '已撤销: '+e.name;
}}
...
function exportCSV() {{
    ...
    a.href = url; a.download = 'annotations_v4.csv'; a.click();
...
out = SCRIPT_DIR / 'annotate_cnn_v3.html'
