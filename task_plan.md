...
- 密钥密码 → GitHub Secret `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`（明文另存密码管理器，不入仓库）
...
## 阶段 53 — v1.0.5 发布（更新弹窗显示更新内容）+ 发布包归档 [complete] 2026-06-29

**目标**：发布 1.0.5，且让旧版用户「检查更新」弹窗显示真实更新内容（changelog），不再只显示版本号一行。

### 完成项
1. **版本号 → 1.0.5**：tauri.conf.json / Cargo.toml / Cargo.lock 三处（package.json 0.1.0 与发版解耦，不动）。
2. **changelog → notes 链路打通**：
   - 链路本就通——弹窗 `update-modal-notes`(pre-wrap+滚动) ← main.ts:856 `info.notes` ← lib.rs:712 `check_for_update` 透传 `u.body` ← latest.json `notes`。**唯一缺口=CI 把 notes 写死 `VoxAlic v{ver}`**。
   - 新增仓库根 `CHANGELOG.md` 作唯一 notes 来源（`## v1.0.5` 段 + 6 条 bullets）。
   - `release.yml` 加「Extract release notes」步骤：正则 `(?ms)^##\s+vX.Y.Z\s*\r?\n(.*?)(?=^##\s+v|\z)` 抽当前版本段 → 写 `release_notes.txt`(UTF8 无 BOM)；GitHub latest.json + Gitee latest.json + Gitee release body + GitHub release `body_path` 全读它；抽不到回退纯版本号。
   - `release_notes.txt` 已 gitignore。本地 PowerShell 验证正则正确截取 v1.0.5 段。
3. **提交发布**：commit `65f94c2 feat:订阅托盘提醒…` + `5a02b29 v1.0.5`，push origin master + tag v1.0.5 → CI 触发，GitHub/Gitee 两边均出 1.0.5 release。
4. **发布包归档**：nsis 文件夹缺 1.0.4（CI 云端构建本地不留），从 GitHub release 下载原版补齐；1.0.5 CI 出包后同样拉入。现归档 1.0.0/1.0.1/1.0.1_dev/1.0.2/1.0.3/1.0.4/1.0.5。

### 新固定动作（memory: feedback-archive-release-installers）
每次发版后检查 nsis 文件夹有无上版安装包，缺了从 GitHub release 拉**原版**（勿重建）归档。

### 待用户核对
- 用 1.0.4「检查更新」→ 弹窗是否显示那 6 条更新内容（CHANGELOG→notes 验证点）。
- GitHub+Gitee 两边 release 完整性（exe+sig+latest.json）。
- 更新后功能正常（订阅托盘红点/裂缝🔔/母亲石印/版本标注）。

### memory 更新
- 新增 `feedback-archive-release-installers.md` + MEMORY.md 索引。
- 更新 `project-voxalic-release.md`：changelog 弹窗已落地 + 发版 checklist（含 CHANGELOG.md 新段）。
